...
当前展示板包含 40 个阵列 case，加 1 个 demo board 真实 through pin-via 对照。

最新 DRC report 结果：

- `Total DRC Errors = 34`
- 34 条详细 DRC 全部为 `Duplicate Drill Hole`
...
- `drill_origin` 对 slot 也应使用 Allegro padstack/drill 定义中的有效 origin，而不是 copper pad 外形中心推测值。
...
- slot 方向/旋转角未穷举；当前 slot case 使用生成 padstack 的默认方向。
