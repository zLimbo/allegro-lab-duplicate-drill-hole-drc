...
当前展示板包含 62 个阵列 case，加 1 个 demo board 真实 through pin-via 对照。

最新 DRC report 结果：

- `Total DRC Errors = 38`
- 38 条详细 DRC 全部为 `Duplicate Drill Hole`
...
新增 R10/M showcase cases 使用 `multiDrillData` 生成 1x2、2x2 circular PTH multi-drill padstack，用来验证“必须全部重合才报，还是局部孔位重合也报”。
...
- 1x2 multi-drill vs single round drill，single 偏移到名义上的左/右 member hole 位置
- 2x2 multi-drill vs single round drill，single 偏移到名义上的 corner member hole 位置
- 相同 1x2 multi-drill 阵列偏移一个 pitch，仅可能局部 member hole 重合
- 相同 2x2 multi-drill 阵列偏移一个 X pitch，仅可能局部 member hole 重合
- 2x2 multi-drill vs 1x2 multi-drill，同 padstack origin 但 pattern shape 不同
- 1x2 pitch-0.50 vs 1x2 pitch-0.60，同 padstack origin 但 pitch 不同

结论：当前证据更支持 Allegro 将 multi-drill padstack 作为“pattern-level drill definition”参与 DH 判定，而不是把 multi-drill 展开成多个独立 drill-hole point 后逐孔比较。也就是说，局部 member hole 重合不够；同 origin 也不够，multi-drill pattern 本身还需要匹配。
...
for each pair of drill_objects:
    ...
    if xy_a == xy_b
       ...
       and multidrill_pattern_compatible(a, b):
...
- `multidrill_pattern_compatible` 对普通 single drill 可视为 true；对 multi-drill 当前实测应要求 pattern 兼容，至少包括 row/column count 和 pitch 等定义匹配。
...
- multi-drill 已覆盖生成的 1x2、2x2 circular PTH 阵列；staggered multi-drill、slot multi-drill、生产库复杂机械孔对象尚未穷举。
