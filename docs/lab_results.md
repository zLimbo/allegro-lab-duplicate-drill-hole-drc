...
当前展示板包含 79 个阵列 case，加 1 个 demo board 真实 through pin-via 对照。

最新 DRC report 结果：

- `Total DRC Errors = 45`
- 45 条详细 DRC 全部为 `Duplicate Drill Hole`
...
- `multidrill_parameters_compatible` 对普通 single drill 可视为 true；对 multi-drill 当前实测应要求 rows/columns、row/column orientation、spacing/clearance、drill diameter/derived pitch 等参数兼容。已测 2x2 staggered flag 差异没有抑制 DH，不应先验当作 suppressor。
